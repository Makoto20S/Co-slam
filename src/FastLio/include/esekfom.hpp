...
namespace esekfom
{
	...
	public:
		...
		//ESKF
		//这段程序实现了一个基于**扩展卡尔曼滤波（EKF）**的迭代优化过程。
		//它通过结合Lidar和Leg数据来更新系统状态，并在每次迭代中计算状态更新和协方差矩阵。
		//通过控制最大迭代次数和收敛判断，程序能够保证状态估计的准确性和收敛性。
		void update_iterated_dyn_share_modified(double R, PointCloudXYZI::Ptr &feats_down_body,
												...
												Eigen::Matrix3d Sigma_leg,Eigen::Vector3d z_leg, bool useleg, bool leg_vaild, bool lidar_vaild)
		{
			

			normvec->resize(int(feats_down_body->points.size())); //这行代码会根据点云数据的大小调整normvec的大小

			dyn_share_datastruct dyn_share; //共享的动态数据结构，包含了状态更新和收敛的标志
			dyn_share.valid = true;
			dyn_share.converge = true;
			int t = 0;
                        state_ikfom x_propagated = x_;
			cov P_propagated = P_; //x_ 和 P_ 分别表示系统的状态和协方差矩阵

			vectorized_state dx_new = vectorized_state::Zero(); // 24X1的向量,dx_new 用来存储每次迭代中估计的状态变化（大小为24x1）


			for (int i = -1; i < maximum_iter; i++)	 // maximum_iter是卡尔曼滤波的最大迭代次数
			{  //每次迭代会更新状态（x_）和协方差矩阵（P_），直到收敛或达到最大迭代次数
				...
				//计算点云数据的雅克比矩阵（h_x）和残差（h）。h_x 用于计算与量测值相关的增量，h 是点云残差
				...
				x_ = boxplus(x_, dx_);	//公式(18),更新x_,收敛判断是通过dx_来完成的
