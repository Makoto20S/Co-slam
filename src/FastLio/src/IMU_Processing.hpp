...
class ImuProcess
{
 public:
  ...
  void Reset();
  void set_param(const V3D &transl, const M3D &rot, const V3D &gyr, const V3D &acc, const V3D &gyr_bias, const V3D &acc_bias, const int imu_init_time);
...
//传入外部参数
void ImuProcess::set_param(const V3D &transl, const M3D &rot, const V3D &gyr, const V3D &acc, const V3D &gyr_bias, const V3D &acc_bias, const int imu_init_time)
{
...
void ImuProcess::IMU_init(const MeasureGroup &meas, esekfom::esekf &kf_state, int &N)
{
  ...
  if (imu_init_th < 2) {
    V3D g_avg = V3D(0, 0, -1.0);
    init_state.grav = - g_avg * G_m_s2;    //得平均测量的单位方向向量 * 重力加速度预设值
    init_state.bg  = Eigen::Vector3d(0,0,0);      //角速度测量作为陀螺仪偏差
  } else {
...
{    //meas:包含了一帧的传感器数据(imu和雷达点云)  kf_state：扩展卡尔曼滤波器的状态对象（保存当前状态估计和协方差） cur_pcl_un_：输出参数，指向“去畸变”后的点云数据
  // T1 = omp_get_wtime();

  if(meas.imu.empty()) {return;}; //**检查IMU数据是否为空
  ROS_ASSERT(meas.lidar != nullptr); //确保必须有有效的激光雷达数据

  if (imu_need_init_)  //**检查IMU是否需要初始化
  {
  ...
  UndistortPcl(meas, kf_state, *cur_pcl_un_);  //**去畸变点云数据,输出到cur_pcl_un_
