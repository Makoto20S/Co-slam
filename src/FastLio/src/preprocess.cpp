...
void Preprocess::avia_handler(const livox_ros_driver::CustomMsg::ConstPtr &msg)
{
  ...
  else
  {
    for (uint i = 1; i < plsize; i++)
    {
      if ((msg->points[i].line < N_SCANS) && ((msg->points[i].tag & 0x30) == 0x10 || (msg->points[i].tag & 0x30) == 0x00))
      {
        ...
        if (valid_num % point_filter_num == 0)
        {
          ...
          if(range == range)
          {
            if ((abs(pl_full[i].x - pl_full[i - 1].x) > 1e-7) || (abs(pl_full[i].y - pl_full[i - 1].y) > 1e-7) || (abs(pl_full[i].z - pl_full[i - 1].z) > 1e-7) && (range > (blind * blind)))
            {
    ...
    // cout<<"points nums:"<<pl_surf.size()<<endl;
    // cout<<"invalid nums:"<<invalid_num<<endl;
  }
}
...
void Preprocess::oust64_handler(const sensor_msgs::PointCloud2::ConstPtr &msg)
{
  ...
  pl_corn.clear();
  pl_full.clear();
  pcl::PointCloud<ouster33_ros::Point> pl_orig;
...
void Preprocess::rs_handler(const sensor_msgs::PointCloud2_<allocator<void>>::ConstPtr &msg)
{
  ...
  int plsize = pl_orig.points.size();
  pl_surf.reserve(plsize);
  std::vector<int> ring_count(32,0);
  /*** These variables only works when no point timestamps given ***/
  double omega_l = 0.361 * SCAN_RATE; // scan angular velocity
  std::vector<bool> is_first(N_SCANS, true);
  std::vector<double> yaw_fp(N_SCANS, 0.0);   // yaw of first scan point
  ...
  for (int i = 0; i < plsize; i++)
  {
    ring_count[pl_orig.points[i].ring]++;
    if(ring_count[pl_orig.points[i].ring] % point_filter_num != 0)
      continue;
    
    double range = pl_orig.points[i].x * pl_orig.points[i].x + pl_orig.points[i].y * pl_orig.points[i].y + pl_orig.points[i].z * pl_orig.points[i].z;

    //判断range是否有效并且去除地面点
    if(range == range && pl_orig.points[i].ring > 15)
    {
      //计算所有点的距离的均值
      if(avg_n == 0)
      {
        avg_n++;
        avg_range = sqrt(range);
        // avg_range = range;
      }
      else
      {
        avg_n++;
        avg_range += (sqrt(range) - avg_range) / avg_n;
        // avg_range += (range - avg_range) / avg_n;
      }
    }
    ...
    if (range > (blind * blind) )
    {
      if(range < (100.0*100.0))//滤掉100m以外的点
      {
        pl_surf.points.push_back(added_pt);
      }
    }
  ...
  //     if (added_pt.x * added_pt.x + added_pt.y * added_pt.y + added_pt.z * added_pt.z > (blind * blind) )
  //     {
  //       if(added_pt.x * added_pt.x + added_pt.y * added_pt.y + added_pt.z * added_pt.z < (100.0*100.0))//滤掉100m以外的点
  //         pl_surf.points.push_back(added_pt);
  //     }
  //   }
  }
...
double Preprocess::get_avg_range()
{
	return avg_range;
}
