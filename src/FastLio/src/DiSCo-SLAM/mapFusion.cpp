...
//    lio_sam::context_info _context_info;
    ...
    std::vector<ScanContextBin> _context_list_to_publish_3;
    ...
    pcl::PointCloud<LwType>::Ptr _laser_Lw_pt;
...
public:
    ...
    void publishContextInfoThread(){
        ...
        int signal_id_th_3 = robotID2Number(_signal_id_3);
        while (ros::ok())
        {
            ...
            if (_communication_signal && _signal_3 && _robot_id_th < signal_id_th_3){
                if (_context_list_to_publish_3.empty())
                    continue;
                //publish scan context info to other robots
                mtx_publish_3.lock();
                ScanContextBin bin = _context_list_to_publish_3.back();
                _context_list_to_publish_3.pop_back();
                mtx_publish_3.unlock();
                publishContextInfo(bin, _signal_id_3);
            }
        }
    }
...
private:
    void ParamLoader(){
        ...
        n.param<std::string>("id_3",  _signal_id_3, "jackal3");
        n.param<double>("sensor_height", sensor_height, 0.0);
    ...
    void initialization(){
        _laser_Lw_pt.reset(new pcl::PointCloud<LwType>());
        ...
        _scan_context_factory = new ScanContext(_max_range, _knn_feature_dim, _num_sectors, sensor_height);
...
//        pcl::fromROSMsg(msgIn->cloud_deskewed, *_laser_Lw_pt);

//        for (int num = 0; num < _laser_Lw_pt->points.size(); ++num) {
//           _laser_cloud_sum->header = _laser_Lw_pt->header;
//           PointType new_pt;
//           new_pt.x = _laser_Lw_pt->points.at(num).x / 1000.0;
//           new_pt.y = _laser_Lw_pt->points.at(num).y / 1000.0;
//           new_pt.z = _laser_Lw_pt->points.at(num).z / 1000.0;
//           _laser_cloud_sum->points.push_back(new_pt);
//        }

//        pcl::fromROSMsg(msgIn->cloud_corner, *_laser_cloud_corner);
        _laser_cloud_corner->points.clear();
//        pcl::fromROSMsg(msgIn->cloud_deskewed, *_laser_cloud_surface);
//        *_laser_cloud_feature += *_laser_cloud_corner;
//        *_laser_cloud_feature += *_laser_cloud_surface;
        *_laser_cloud_feature += *_laser_cloud_sum;
//        std::cout<<"feature point num is :"<<_laser_cloud_feature->points.size()<<";cloud_deskewed size is: "<<_laser_cloud_sum->points.size()<<std::endl;
        ...
        mtx_publish_2.unlock();
        mtx_publish_3.lock();
        _context_list_to_publish_3.push_back(bin);
        mtx_publish_3.unlock();
...
//        return;
    ...
    void gtsamFactorGraph(){
        ...
        if (_global_map_trans.size() != 0)
            id_received.push_back(_robot_id_th);//添加自身的id
    ...
    void buildKDTree(ScanContextBin bin){
        ...
        _target_matrix.conservativeResize(_knn_feature_dim, _num_bin);//_knn_feature_dim = 64;64*num
...
//            if (sqrt(pow((bin.pose.x - _bin_with_id.at(idx_candidate).pose.x), 2)+ pow((bin.pose.y - _bin_with_id.at(idx_candidate).pose.y), 2)) > 1
//                    || abs(bin.pose.z - _bin_with_id.at(idx_candidate).pose.z) > 0.5)
//                continue;
            //add to idx list
        ...
        for (int i = 0; i < std::min( _num_match_candidates, int(idx_list.size()) ); i++){//_num_match_candidates = 1;会找到一个最优的闭环
    ...
    bool getInitialGuess(ScanContextBin bin, int idx_nearest, int min_idx){
        ...
        if (pose_source_lidar.intensity == -1 || pose_source_lidar.intensity > _icp_thres)
            return false;
//        std::cout<<_robot_id_th<<" reveive loop from "<<_robot_this_th<<"; FitnessScore is : "<<pose_source_lidar.intensity<<std::endl;
...
//        }
    ...
    void update_loop_info(int id_bin_last, int id_bin_this, gtsam::Pose3 pose_to_last, gtsam::Pose3 pose_to_this, float intensity){
        ...
        _loop_info.numRing   = _bin_with_id[id_bin_last].pose.intensity;
        _loop_info.numSector = _bin_with_id[id_bin_this].pose.intensity;
