...
ScanContext::ScanContext(int max_range, int num_rings, int num_sectors, double sensor_height)
  : _max_range(max_range), _num_rings(num_rings), _num_sectors(num_sectors), _sensor_height(sensor_height){
...
ScanContextBin ScanContext::ptcloud2bin(pcl::PointCloud<PointType>::Ptr pt_cloud){
  ...
  sc_bin.bin = ptCloud2ScanContext(sc_bin.cloud);//存储最大z值的sc特征矩阵
  sc_bin.ringkey = scanContext2RingKey(sc_bin.bin);//行向量范数构成的列向量
  return sc_bin;
}
...
Eigen::MatrixXf ScanContext::ptCloud2ScanContext(pcl::PointCloud<PointType>::Ptr pt_cloud){
  ...
  for (int i = 0; i < num_points; ++i){
    ...
    int ring_index = floor(range/_gap);//行索引值
    if (ring_index < 0) 
        continue; 
    ...
    int sector_index = ceil(theta/_angle_one_sector);//列索引值
    if (sector_index == 0)
      continue;
    ...
    if (point_this.z > max_bin(ring_index, sector_index))
      max_bin(ring_index, sector_index) = point_this.z;//存储最大z值
  ...
  for (int i = 0; i < _num_rings; i++){
    for (int j = 0; j < _num_sectors; j++){
      if (max_bin(i, j) != 0)
        max_bin(i, j) = max_bin(i, j) + _sensor_height;
